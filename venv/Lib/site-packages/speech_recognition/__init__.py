...
class AudioData(object):
    ...
    def get_flac_data(self, convert_rate=None, convert_width=None, flac_converter=None):
        """
        ...
        flac_converter = flac_converter or get_flac_converter()
        if os.name == "nt":  # on Windows, specify that the process is to be started without showing a console window
            startup_info = subprocess.STARTUPINFO()
            startup_info.dwFlags |= subprocess.STARTF_USESHOWWINDOW  # specify that the wShowWindow field of `startup_info` contains a value
            startup_info.wShowWindow = subprocess.SW_HIDE  # specify that the console window should be hidden
...
class Recognizer(AudioSource):
    ...
    def recognize_google(self, audio_data, key=None, language="en-US", show_all=False, flac_converter=None):
        """
        ...
        flac_data = audio_data.get_flac_data(
            ...
            flac_converter=flac_converter  # path of the flac executable file
        )
...
def get_flac_converter():
    ...
    if flac_converter is None:  # flac utility is not installed
        base_path = os.path.dirname(os.path.abspath(base_path or __file__))  # directory of the current module file, where all the FLAC bundled binaries are stored
