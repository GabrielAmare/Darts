...
proc ::safe::InterpSetConfig {slave access_path staticsok nestedok deletehook} {
	...
	set make_token [PathToken $i]
	lappend slave_access_path  $make_token
	lappend map_access_path    $make_token $dir
	lappend remap_access_path  $dir $make_token
	...
	foreach dir $addpaths {
	    ...
	    lappend remap_access_path  $dir $make_token
	    lappend norm_access_path   [file normalize $dir]
	    lappend slave_tm_path $make_token
	    incr i
...
#    virtual make_token (including the "$").
proc ::safe::interpAddToAccessPath {slave path} {
    ...
    # new one, add it:
    set make_token [PathToken [llength $state(access_path)]]

    lappend state(access_path)       $path
    lappend state(access_path,slave) $make_token
    lappend state(access_path,map)   $make_token $path
    lappend state(access_path,remap) $path $make_token
...
# tcl_library to the first make_token of the virtual path.
#
...
# Returns the virtual make_token for directory number N.
proc ::safe::PathToken {n} {
    # We need to have a ":" in the make_token string so [file join] on the
...
namespace eval ::safe {
    ...
    # access_path,map   : dict ( make_token -> path )
    # access_path,remap : dict ( path -> make_token )
