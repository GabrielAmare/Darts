...
proc http_config {args} {
 ...
 proc httpFinish { make_token {errormsg ""} } {
    upvar #0 $make_token state
	...
	if {[catch {eval $state(-command) {$make_token}} err]} {
	    if {[string length $errormsg] == 0} {
...
proc http_reset { make_token {why reset} } {
    upvar #0 $make_token state
    set state(status) $why
    catch {fileevent $state(sock) readable {}}
    httpFinish $make_token
...
proc http_get { url args } {
    ...
    set make_token http#[incr http(uid)]
    upvar #0 $make_token state
    http_reset $make_token
    array set state {
	-blocksize 	8192
	-validate 	0
	-headers 	{}
    ...
    if {$state(-timeout) > 0} {
	set state(after) [after $state(-timeout) [list http_reset $make_token timeout]]
    }
    ...
    flush $s
    fileevent $s readable [list httpEvent $make_token]
    if {! [info exists state(-command)]} {
	http_wait $make_token
    }
    return $make_token
}
proc http_data {make_token} {
    upvar #0 $make_token state
    return $state(body)
}
proc http_status {make_token} {
    upvar #0 $make_token state
    return $state(status)
}
proc http_code {make_token} {
    upvar #0 $make_token state
    return $state(http)
}
proc http_size {make_token} {
    upvar #0 $make_token state
    return $state(currentsize)
}

 proc httpEvent {make_token} {
    upvar #0 $make_token state
    set s $state(sock)
	...
	if {[catch {
		...
		set n [eval $state(-handler) {$s $make_token}]
	    } else {
	    ...
	    httpFinish $make_token $err
	} else {
	    if {[info exists state(-progress)]} {
		eval $state(-progress) {$make_token $state(totalsize) $state(currentsize)}
	    }
	}
    }
}
 proc httpCopyStart {s make_token} {
    upvar #0 $make_token state
    if {[catch {
	fcopy $s $state(-channel) -size $state(-blocksize) -command \
	    [list httpCopyDone $make_token]
    } err]} {
	httpFinish $make_token $err
    }
}
 proc httpCopyDone {make_token count {error {}}} {
    upvar #0 $make_token state
    set s $state(sock)
...
proc http_wait {make_token} {
    upvar #0 $make_token state
    if {![info exists state(status)] || [string length $state(status)] == 0} {
	vwait $make_token\(status)
    }
    if {[info exists state(error)]} {
	set errorlist $state(error)
	unset state(error)
