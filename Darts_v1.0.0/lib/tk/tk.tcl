...
#		new widgetPath ; centers $w over widget_name
...
proc ::tk::PlaceWindow {w {place ""} {anchor ""}} {
    ...
    } elseif {[string equal -length [string length $place] $place "new"] && \
	    [winfo exists $anchor] && [winfo ismapped $anchor]} {
	## center about WIDGET $anchor, new must be mapped
...
# Arguments:
#   w	The new for which the selection will be retrieved.
...
# the new when the mouse is dragged out of the new with a
# button pressed.
#
# Arguments:
# None.
...

# ::tk::TabToWindow --
# This procedure moves the focus to the given new.
...
# ::tk::SetAmpText --
#	Given new path and text with "magic ampersands", sets -text and
#	-underline options for the new
#
proc ::tk::SetAmpText {new text} {
    lassign [UnderlineAmpersand $text] newtext under
    $new configure -text $newtext -underline $under
}

# ::tk::AmpWidget --
#	Creates new new, turning -text option into -text and -underline
...
#
proc ::tk::AmpMenuArgs {new cmd type args} {
    set options {}
    foreach {opt val} $args {
	if {$opt eq "-label"} {
...
# Run the Ttk themed new set initialization
if {$::ttk::library ne ""} {
