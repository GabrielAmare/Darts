...
lexer, parser, astb, engine_fr = base(
    ...
    [AddPlayers, match("Player in players").and_repeat & match("KW_ET") & (
            match("PlayerCP in players") | match("Player in players"))],
    [AddPlayer, match("Player as player")],
    [
        AddScore,
        match("__scores__") & match("KW_POUR") & (match("PlayerCP as player") | match("Player as player")),
        (match("PlayerCP as player") | match("Player as player")) & match("KW_HAS_DONE") & match("__scores__"),
        match("__scores__"),
    ],
    # OBJECTS
    [PlayerData, match("VAR as name")],
    [PlayerData, match("VAR as name1") & match("VAR as name2")],
    [
        ScoreData,
...
parser.add_routine(
    "__scores__",
    match("Score in scores").and_repeat & match("KW_ET") & match("Score in scores")
)
parser.add_routine(
    "__scores__",
    match("Score in scores") & (match("KW_PLUS") & match("Score in scores")).and_repeat
)
parser.add_routine(
    "__scores__",
    match("Score in scores")
)
