from tklib37 import *
from .Party_Molkky import Party_Molkky
from ...app_config import STYLE


class ScoreBoard_Molkky(KeyTable):
    LABEL_CFG = STYLE.SCOREBOARD.MOLKKY.LABEL
    ...
    def on_append_score(self, **cfg):
        score = cfg["item"]
        if score.player.name in self.col_map:
            self.set_widget(row=1, col=score.player.name, text=score.player.scores[-1].score)
            self.update_idletasks()
...
Party_Molkky.tk_scoreboard = lambda party, root, **config: ScoreBoard_Molkky(root, party, **config)
